    ...
    for idx = 1:length(EbN0)
        ...
        for mRx = 1:M
            for n=1:4
                ...
                r1(:,n) = awgn(sum(H.*x1,2)/sqrt(N), EbN0(idx) - NF,'measured');
                r2(:,n) = awgn(sum(H.*x2,2)/sqrt(N), EbN0(idx) - NF,'measured');
                r3(:,n) = awgn(sum(H.*x3,2)/sqrt(N), EbN0(idx) - NF,'measured');
                r4(:,n) = awgn(sum(H.*x4,2)/sqrt(N), EbN0(idx) - NF,'measured');
                r5(:,n) = awgn(sum(H.*x5,2)/sqrt(N), EbN0(idx) - NF,'measured');
                r6(:,n) = awgn(sum(H.*x6,2)/sqrt(N), EbN0(idx) - NF,'measured');
                r7(:,n) = awgn(sum(H.*x7,2)/sqrt(N), EbN0(idx) - NF,'measured');
                r8(:,n) = awgn(sum(H.*x8,2)/sqrt(N), EbN0(idx) - NF,'measured');
            ...
            r_comb = [ sum(sum(z1,3),2), sum(sum(z2,3),2),sum(sum(z3,3),2), sum(sum(z4,3),2) ]/ sqrt(N*M) ;
...
% Vẽ kết quả
h = figure;
semilogy(EbN0, BER41, 'r*-', 'LineWidth', 1, 'DisplayName', 'Tx=4,Rx=1,STBC 4x4');
hold on;
semilogy(EbN0, BER42, 'bs-', 'LineWidth', 1, 'DisplayName', 'Tx=4,Rx=2,STBC 4x4');
hold on;
semilogy(EbN0, BER43, 'g^-', 'LineWidth', 1, 'DisplayName', 'Tx=4,Rx=3,STBC 4x4');
hold on;
semilogy(EbN0, BER44, 'k>-', 'LineWidth', 1, 'DisplayName', 'Tx=4,Rx=4,STBC 4x4');
grid on;
